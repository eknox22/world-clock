...
function updateCity(event) {
  ...
  let cityName = cityTimeZone.replace('_","").replace');
  if ((cityTimeZone = moment().tz(cityTimeZone)));
  let citiesElement = document.querySelector("#cities");
  citiesElement.innerHTML = `<div class="city">
          <div>
            <h2>${cityName}</h2>
            <div class="date">${cityTimeZone.format("dddd MMM Do YYYY")}</div>
          </div>
          <div class="time">${cityTimeZone.format("HH:mm:ss")}</div>
        </div>`;
}
